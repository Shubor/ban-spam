...
In the extension we implemented seven feature selection metrics in order to the determine which would return the best result. We chose Information gain (IG), Correlation coefficient (CC) and Odds ratio (OR) \cite{IG}, Categorical proportional difference (CPD), Mutual information (MI) \cite{CPD}, Document frequency (DF) and Random feature (RF).
