...
We ran our preprocessed data against a variety of Weka classifiers; ZeroR, OneR, IBk with k=1 (1-NN) and k=3 (3-NN), NaiveBayes (NB), J48 (DT) and MultilayerPerceptron (MLP). We tested these with Weka's 10 fold cross-validation.
...
In this report, we apply Na\"ive Bayes for binary classification of the \emph{LingSpam-mini600} data set. In order to reduce the size of the feature space, we use Document Frequency for feature selection. We have shown using accuracy that our classifier performs extremely well compared to a variety of other classifiers (ZeroR, OneR, 1-NN, 3-NN, NB, DT and MLP), with only MLP getting a slightly better accuracy than ours on the body corpus. Therefore, based on our experimental results, Na\"ive Bayes is well suited for email classification to detect spam. 
