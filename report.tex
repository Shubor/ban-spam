...
We ran our classifier with a number of feature selection methods; correlation coefficient (CC), information gain (IG), mutual information (MI), chi (CHI), document frequency (DF), categorical proportional difference (CPD) and random (RF).
...
Information gain yields the highest accuracy for most of the feature set sizes (\autoref{table:diffbody}). It almost always gives us a higher accuracy than using document frequency (DF). DF is simple, has a linear computational complexity and is easily scalable for larger corpora. However, it removes low DF terms that could be more informative than the ones selected. This is largely why it performs more poorly than IG, as IG measures the number of bits of information gained by including a feature.

Mutual information (MI) and random frequency (RF) both perform weakly. The low accuracy produced by MI is because the score of each term is strongly influenced by marginal probabilities. Therefore, it performs badly when the scores have a wide range of frequencies. It was expected that RF perform poorly, as it randomly chose features.
